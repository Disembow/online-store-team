export default function setDropdown(): void {
    ...
    if (!(dropdowns instanceof NodeList) || dropdowns === null)
        throw new Error('dropdowns is null');

    dropdowns.forEach((dropdown: Element) => {

        const blockSelected = dropdown.querySelector('.dropdown__selected')
        if (!(blockSelected instanceof HTMLElement) || blockSelected === null)
            throw new Error('blockSelected is null');

        const dropdownList = dropdown.querySelector('.dropdown__list')
        if (!(dropdownList instanceof HTMLElement) || dropdownList === null)
            throw new Error('dropdownList is null');

        const dropdownArrow = dropdown.querySelector('.dropdown__arrow')
        if (!(dropdownArrow instanceof HTMLElement) || dropdownArrow === null)
            throw new Error('dropdownArrow is null');

        const dropdownItems = document.querySelectorAll('.dropdown__item')
        if (!(dropdownItems instanceof NodeList) || dropdownItems === null)
            throw new Error('dropdownItems is null');
